	...
	void LaserMonitoringEoP::Init(TTree *tree, TTree *treeMC, int useRegression)
	{
		...
		fChainMC->SetBranchAddress("chargeEle", &chargeEle);
	...
	void LaserMonitoringEoP::Loop( float yMIN, float yMAX, std::string EBEE, int evtsPerPoint, int useRegression, std::string dayMin, std::string dayMax, std::string dayZOOM,
		                           std::string LUMI, int vsPU, int IetaMin, int IetaMax, int IphiMin, int IphiMax, int ee_PNregion, int etaRing_min, int etaRing_max)
	{
		if (fChain == 0) return;
		...
		*/
		...
		std::cout << "etaRing_min: "   << etaRing_min   << std::endl;
		std::cout << "etaRing_max: "   << etaRing_max    << std::endl;
...
/*
		if( (IetaMin != -1000.) && (IetaMax != -1000.) && (IphiMin != -1000.) && (IphiMax != -1000.) ) {
			char absEtaBuffer[50];
...
*/		
		...
		*/
		...
		// if correcting endcaps, get PN regions map
		
		std::string pnregions_map = "/afs/cern.ch/user/v/vciriolo/work/private/EOverP/CMSSW_8_0_24_patch1/src/Calibration/ZFitter/macro/pn_ee.txt";
		if ( EBEE == "EE" )	PN_ee_map = PNregion_ee_map(pnregions_map);
		...
		for(int ientry = 0; ientry < fChainMC->GetEntries(); ++ientry) 
		{
			...
			if ( EBEE == "EE" )
			{
				if (ee_PNregion!= -1)
				{
					//std::cout << "key = " << (seedXSCEle[0]*100 + seedYSCEle[0])*iz  << "   pn region = "  << PN_ee_map[(seedXSCEle[0]*100 + seedYSCEle[0])*iz] << std::endl;
					int iz = -100;
					etaSCEle[0] > 0 ? iz = 1 : iz = -1;
					if (PN_ee_map[(seedXSCEle[0]*100 + seedYSCEle[0])*iz] != ee_PNregion)	continue;
				}
				
				if (etaRing_min != -1 && etaRing_max != -1)
				{
					int iz = -100;
					etaSCEle[0] > 0 ? iz = 1 : iz = -1;
					int iRing = eRings->GetEndcapRing(seedXSCEle[0],seedYSCEle[0], iz ) +1;
					if (iRing < etaRing_min || iRing > etaRing_max)	continue;
				}
			}
			
			else if ( EBEE == "EB" )
			{
				if( (IetaMin != -1000) && (IetaMax != -1000) ) 
				{
					if( (seedXSCEle[0] < IetaMin) || (seedXSCEle[0] > IetaMax) ) continue;			
				
					if ((IphiMin != -1000) && (IphiMax != -1000) )
					{						
			...
			/*
			if (std::abs(chargeEle[1]) != 1)	continue;
			
			if( (strcmp(EBEE.c_str(), "EB") == 0) && (fabs(etaSCEle[1]) > 1.479) )                    continue; // barrel
			if( (strcmp(EBEE.c_str(), "EB_0_1") == 0) && (fabs(etaSCEle[1]) > 1.0)) continue; // inner barrel
			if( (strcmp(EBEE.c_str(), "EB_1_1479") == 0) && (fabs(etaSCEle[1]) < 1.0 || fabs(etaSCEle[1]) > 1.479) )                     continue; // barrel
			if( (strcmp(EBEE.c_str(), "EE") == 0) && (fabs(etaSCEle[1]) < 1.479 || fabs(etaSCEle[1]) > 2.5) ) continue; // endcap
			if( (strcmp(EBEE.c_str(), "EE_1479_2") == 0) && (fabs(etaSCEle[1]) < 1.479 || fabs(etaSCEle[1]) > 2.0) ) continue; // endcap
			if( (strcmp(EBEE.c_str(), "EE_2_25") == 0) && (fabs(etaSCEle[1]) < 2.0 || fabs(etaSCEle[1]) > 2.5) ) continue; // endcap

			if( (strcmp(EBEE.c_str(), "EEp") == 0) && (etaSCEle[1] < 1.479 || etaSCEle[1] > 2.5 ) ) continue; // endcap
			if( (strcmp(EBEE.c_str(), "EEp_1479_2") == 0) && (etaSCEle[1] < 1.479 || etaSCEle[1] > 2.0) ) continue; // endcap
			if( (strcmp(EBEE.c_str(), "EEp_2_25") == 0) && (etaSCEle[1] < 2.0 || etaSCEle[1] > 2.5) ) continue; // endcap
			if( (strcmp(EBEE.c_str(), "EEp_2_225") == 0) && (etaSCEle[1] < 2.0 || etaSCEle[1] > 2.25) ) continue; // endcap
			if( (strcmp(EBEE.c_str(), "EEp_225_25") == 0) && (etaSCEle[1] < 2.25 || etaSCEle[1] > 2.25) ) continue; // endcap

			if( (strcmp(EBEE.c_str(), "EEm") == 0) && (etaSCEle[1] > -1.479 || etaSCEle[1] < -2.5 ) ) continue; // endcap
			if( (strcmp(EBEE.c_str(), "EEm_1479_2") == 0) && (etaSCEle[1] > -1.479 || etaSCEle[1] < -2.0) ) continue; // endcap
			if( (strcmp(EBEE.c_str(), "EEm_2_25") == 0) && (etaSCEle[1] > -2.0 || etaSCEle[1] < -2.5) ) continue; // endcap
			if( (strcmp(EBEE.c_str(), "EEm_2_225") == 0) && (etaSCEle[1] > -2.0 || etaSCEle[1] < -2.25) ) continue; // endcap
			if( (strcmp(EBEE.c_str(), "EEm_225_25") == 0) && (etaSCEle[1] > -2.25 || etaSCEle[1] < -2.5) ) continue; // endcap

			if( (absEtaMin != -1.) && (absEtaMax != -1.) ) 
			{
				if( (fabs(etaSCEle[1]) < absEtaMin) || (fabs(etaSCEle[1]) > absEtaMax) ) continue;
			}
			...
			if ( EBEE == "EE" )
			{
				if (ee_PNregion!= -1)
				{
					//std::cout << "key = " << (seedXSCEle[1]*100 + seedYSCEle[1])*iz  << "   pn region = "  << PN_ee_map[(seedXSCEle[1]*100 + seedYSCEle[1])*iz] << std::endl;
					int iz = -100;
					etaSCEle[1] > 0 ? iz = 1 : iz = -1;
					if (PN_ee_map[(seedXSCEle[1]*100 + seedYSCEle[1])*iz] != ee_PNregion)	continue;
				}
				...
				if (etaRing_min != -1 && etaRing_max != -1)
				{
					int iz = -100;
					etaSCEle[1] > 0 ? iz = 1 : iz = -1;
					int iRing = eRings->GetEndcapRing(seedXSCEle[1],seedYSCEle[1], iz ) +1;
					if (iRing < etaRing_min || iRing > etaRing_max)	continue;
				}
			}
			...
			else if ( EBEE == "EB" )
			{
				if( (IetaMin != -1000) && (IetaMax != -1000) ) 
				{
					if( (seedXSCEle[1] < IetaMin) || (seedXSCEle[1] > IetaMax) ) continue;			
				
					if ((IphiMin != -1000) && (IphiMax != -1000) )
					{						
						if( (seedYSCEle[1] < IphiMin) || (seedYSCEle[1] > IphiMax) ) continue;

					}
				}
			}
			h_template -> Fill( (energySCEle[1] - esEnergySCEle[1]) / (pAtVtxGsfEle[1] - esEnergySCEle[1]) / PUCorr );			
		}
		*/
		...
		//std::vector<bool> isSavedEntries2(nEntries);
		...
		for(int ientry = 0; ientry < nEntries; ++ientry)
		{
			...
			/*
			isSavedEntries2.at(ientry) = false;
			// do the same for the second electron (in case of DoubleEG)
			if (abs(chargeEle[1]) != 1)	continue;
			...
			if( avgLCSCEle[1] <= 0. ) continue;
			isSavedEntries2.at(ientry) = true;
			*/
		}
		...
		for(int i = 0; i < nBins; ++i) {
			...
			*/
			...
			if( (h_EoP[i]->GetEntries() > 0) && (fStatus == 0) ) {
		...
		*/
		...
		if (vsPU == 0) 
		{
			...
			c_scOccupancy -> Print((folderName + "/" + folderName + "_scOccupancy.C").c_str());
			c_seedOccupancy -> Print((folderName + "/" + folderName + "_seedOccupancy.C").c_str());
...
std::map<int,int> LaserMonitoringEoP::PNregion_ee_map(std::string infile)
{
	std::ifstream inmap;
	inmap.open(infile.c_str());
	std::string ix = "", iy = "", iz = "", detId = "", pnA = "", pnB = "";
	std::map<int,int> pn_map;
	
	while(!inmap.eof())
	{
		inmap >> detId >> ix >> iy >> iz >> pnA >> pnB;
		pn_map.insert(std::pair<int,int>( (std::atoi(ix.c_str())*100 + std::atoi(iy.c_str()))*std::atoi(iz.c_str()) , std::atoi(pnA.c_str())));
	}
