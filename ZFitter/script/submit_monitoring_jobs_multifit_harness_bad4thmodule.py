...
os.system("mkdir   Job_monitoring_badRegions"+ntupleName)

ietamin   = ["65","65","65","65"]
iphimax=   ["150","120","170","180"]
ietamax   =   ["85","85","85","85"]
iphimin   =   ["140","110","160","170"]

#run = ["runB_v1","runB_v2","runC_v1","runC_v2","runC_v3","runD_v1"]

for   i   in   range(len(categories)):
  for k in range(len(ietamin)):
       #for j in range(len(run)):
       #fn   =   "Job_monitoring_"+ntupleName+"/Job_"+categories[i]+"_ietamin_"+str(ietamin*(1))+"_ietamax_"+str(ietamax*(1))+"_iphimin_"+str(iphimin)+"_iphimax_"+str(iphimax)+"_1nw";
       fn   =   "Job_monitoring_badRegions"+ntupleName+"/Job_"+categories[i]+"_ietamin_"+str(ietamax[k])+"_ietamax_"+str(ietamin[k])+"_iphimin_"+str(iphimin[k])+"_iphimax_"+str(iphimax[k]);
...
#            command   =   "ZFitter.exe   -f   EoverPmonitoring_batch_"+ntupleName+".dat   --evtsPerPoint   200000   --laserMonitoringEP   --EBEE   "+categories[i]+"   --yMIN   0.8   --yMAX   1.20   --LUMI   24.1   --dayMin   15-03-2016   --dayMax   30-12-2016   --noPU"   #use   this   if   you   want   to   run   E/p   vs   LC
       ...
       command   =   "ZFitter.exe   -f   /afs/cern.ch/user/v/vciriolo/work/private/EOverP/CMSSW_8_0_24_patch1/src/Calibration/ZFitter/data/validation/"+ntupleName+".dat   --evtsPerPoint   "+events[i]+"   --laserMonitoringEP   --EBEE   "+categories[i]+"   --yMIN   "+yMIN[i]+"   --yMAX   "+yMAX[i]+"   --LUMI   17.8   --dayMin   15-03-2016   --dayMax   30-12-2017   --noPU   --IetaMax   "+   str(ietamax[k])   +   "   --IetaMin   "   +   str(ietamin[k])   +   "      --IphiMin   "   +   str(iphimin[k])   +   "   --IphiMax   "   +   str(iphimax[k])   #default,   to   be   run   for   E/p   vs   time  
       print   command;
...
#                                                                        outScript.write("\ncp   -v   /afs/cern.ch/user/l/lbrianza/work/public/ntupleEoP/*   .")
       ...
       outScript.write("\ncp   -v   -r   "+categories[i]+"__/   /eos/user/v/vciriolo/EoP_monitoring/harness2017/Cal_Oct2017/SingleAndDouble/Cal_Oct2017_Ped_v2/"+categories[i]+"_"+ntupleName+"_badRegions_ietamax_"+str(ietamin[k])+"_ietamin_"+str(ietamax[k])+"_iphimin_"+str(iphimin[k])+"_iphimax_"+str(iphimax[k])+"/")
