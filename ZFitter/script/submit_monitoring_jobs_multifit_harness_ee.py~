...
categories = ["EE"]#,"EB_0_1","EB_1_1479"]#,"EE","EE_1479_2","EE_2_25","EEp","EEp_1479_2","EEp_2_25","EEm","EEm_1479_2","EEm_2_25","EEp_2_225","EEp_225_25","EEm_2_225","EEm_225_25"]
yMIN = ["0.40"]#,"0.85","0.85"]#,"0.65","0.65","0.40","0.65","0.65","0.40","0.65","0.65","0.40","0.40","0.40","0.40","0.40"]
...
for i in range(len(categories)):
 for n in range(0, 7):
  ...
  for j in range(10,20):
   ee_PNregion = j + n *10
   #fn = "Job_monitoring_"+ntupleName+"/Job_"+categories[i]+"_ietamin_"+str(ietamin*(1))+"_ietamax_"+str(ietamax*(1))+"_iphimin_"+str(iphimin)+"_iphimax_"+str(iphimax);
   fn = "Job_monitoring_"+ntupleName+"/Job_"+categories[i]+"_ee_PNRegion"+str(ee_PNregion);
...
#    command = "ZFitter.exe -f EoverPmonitoring_batch_"+ntupleName+".dat --evtsPerPoint 200000 --laserMonitoringEP --EBEE "+categories[i]+" --yMIN 0.8 --yMAX 1.20 --LUMI 24.1 --dayMin 15-03-2016 --dayMax 30-12-2016 --noPU" #use this if you want to run E/p vs LC
   ...
   command = "ZFitter.exe -f /afs/cern.ch/user/v/vciriolo/work/private/EOverP/CMSSW_8_0_24_patch1/src/Calibration/ZFitter/data/validation/"+ntupleName+".dat --evtsPerPoint "+events[i]+" --laserMonitoringEP --EBEE "+categories[i]+" --yMIN "+yMIN[i]+" --yMAX 1.15 --LUMI 17.9 --dayMin 01-01-2017 --dayMax 30-12-2017 --noPU --ee_PNregion "+str(ee_PNregion) #default, to be run for E/p vs time
   print command;
...
#                        outScript.write("\ncp -v /afs/cern.ch/user/l/lbrianza/work/public/ntupleEoP/* .")
   ...
   outScript.write("\ncp -v -r "+categories[i]+"__/ /eos/user/v/vciriolo/EoP_monitoring/harness2017/Cal_Oct2017/SingleAndDouble/Cal_Oct2017_Ped_v2/"+categories[i]+"_"+ntupleName+"_PNregion_"+str(ee_PNregion)+"/")
   #outScript.write("\ncp -v -r "+categories[i]+"__/ /eos/user/v/vciriolo/EoP_monitoring/harness2017/Cal_Oct2017/SingleAndDouble/Cal_Oct2017_Ped_v2/1nw/"+categories[i]+"_"+ntupleName+"_PNregion_"+str(ee_PNregion)+"/")
   outScript.close();
